...
export default function DestinationEditorForm({
  ...
  async function save(status?: DestinationRecord["status"]) {
    ...
    setMessage("");
    try {
      const response = await fetch(mode === "create" ? "/api/admin/destinations" : `/api/admin/destinations/${draft.id}`, {
      ...
      if (mode === "create") {
        router.push(`/admin/destinations/${data.id}/edit`);
        return;
      }
      setDraft(data.destination);
  ...
  async function remove() {
    ...
    setSaving(true);
    try {
      const response = await fetch(`/api/admin/destinations/${draft.id}`, { method: "DELETE" });
      const data = await response.json().catch(() => ({}));
    ...
    <main className="min-h-screen px-6 py-10 text-white">
      <div className="mx-auto max-w-5xl space-y-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="text-xs uppercase tracking-[0.28em] text-[#ffb066]">Destination Editor</div>
            <h1 className="mt-3 text-3xl font-semibold">{draft.title || "Untitled destination"}</h1>
            <p className="mt-3 text-sm text-white/60">Control the published destination card without changing the public Gene feel.</p>
          </div>
