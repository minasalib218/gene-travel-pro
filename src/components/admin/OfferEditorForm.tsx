...
import type { OfferLiveRecord } from "@/lib/content/offers-live";
...
export default function OfferEditorForm({
  ...
  async function save(status?: OfferLiveRecord["status"]) {
    ...
    setMessage("");
    try {
      const response = await fetch(mode === "create" ? "/api/admin/offers" : `/api/admin/offers/${draft.id}`, {
  ...
  async function remove() {
    ...
    setSaving(true);
    try {
      const response = await fetch(`/api/admin/offers/${draft.id}`, { method: "DELETE" });
      const data = await response.json().catch(() => ({}));
    ...
    <main className="min-h-screen px-6 py-10 text-white">
      <div className="mx-auto max-w-5xl space-y-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="text-xs uppercase tracking-[0.28em] text-[#ffb066]">Offer Editor</div>
            <h1 className="mt-3 text-3xl font-semibold">{draft.title || "Untitled offer"}</h1>
          </div>
        ...
        <section className="rounded-[34px] border border-white/10 bg-white/[0.04] p-6 shadow-[0_24px_70px_rgba(0,0,0,0.28)] backdrop-blur-2xl">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Field label="Duration"><input value={draft.duration} onChange={(e) => setDraft({ ...draft, duration: e.target.value })} className="input" /></Field>
            <Field label="Starting Price"><input value={draft.startingPrice} onChange={(e) => setDraft({ ...draft, startingPrice: e.target.value })} className="input" /></Field>
            <Field label="Discount Badge"><input value={draft.discountBadge} onChange={(e) => setDraft({ ...draft, discountBadge: e.target.value })} className="input" /></Field>
            <Field label="Expires At"><input type="date" value={draft.expiresAt ? String(draft.expiresAt).slice(0, 10) : ""} onChange={(e) => setDraft({ ...draft, expiresAt: e.target.value })} className="input" /></Field>
            <AdminImageUploadField label="Card Image" bucket="offers" value={draft.imageUrl} onChange={(value) => setDraft({ ...draft, imageUrl: value })} />
            <AdminImageUploadField label="Icon Image" bucket="offers" value={draft.iconUrl} onChange={(value) => setDraft({ ...draft, iconUrl: value })} />
            ...
            <Field label="Status">
              <select value={draft.status} onChange={(e) => setDraft({ ...draft, status: e.target.value as OfferLiveRecord["status"] })} className="input">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <label className="flex items-center gap-3 text-sm text-white/78"><input type="checkbox" checked={draft.featured} onChange={(e) => setDraft({ ...draft, featured: e.target.checked })} /> Featured</label>
        ...
        <style jsx global>{`
          .input { width:100%; border-radius:1rem; border:1px solid rgba(255,255,255,0.1); background:rgba(0,0,0,0.3); padding:0.85rem 1rem; color:white; }
        `}</style>
      </div>
    </main>
  );
