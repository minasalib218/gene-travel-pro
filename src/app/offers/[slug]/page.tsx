    ...
    <main className="min-h-screen bg-[#050505] text-white">
      ...
      <section className="relative min-h-[88svh] overflow-hidden">
        <div className="absolute inset-0">
          <Image src={offer.imageUrl || "/images/customize-band.jpg"} alt={offer.title} fill priority className="object-cover" />
        ...
        <div className="relative mx-auto max-w-7xl px-5 pb-16 pt-32 md:px-8">
          <div className="grid gap-10 lg:grid-cols-[1fr_0.9fr] lg:items-end">
            <div className="max-w-2xl">
              <div className="text-xs uppercase tracking-[0.32em] text-[#ffc785]">Offer In {offer.location || offer.country}</div>
              <h1 className="mt-5 text-5xl font-semibold leading-[0.92] md:text-7xl">{offer.title}</h1>
              <p className="mt-5 max-w-xl text-base leading-8 text-white/70">{offer.description}</p>
              <div className="mt-8 flex flex-wrap gap-4">
                <GlassInfo label="Destination" value={[offer.location, offer.country].filter(Boolean).join(", ") || "Destination"} />
                <GlassInfo label="Offer Type" value={offer.discountBadge || "Limited offer"} />
                <GlassInfo label="Current Price" value={offer.startingPrice || "Custom pricing"} />
              </div>
              <div className="mt-10 flex flex-wrap gap-4">
                ...
                <Link href="/offers" className="rounded-full border border-white/20 bg-white/[0.05] px-6 py-3 text-xs uppercase tracking-[0.2em] text-white/82 transition hover:bg-white/[0.1]">Back To Offers</Link>
              </div>
            </div>
            <div className="rounded-[34px] border border-white/10 bg-white/[0.06] p-6 shadow-[0_24px_70px_rgba(0,0,0,0.26)] backdrop-blur-xl">
              <div className="text-xs uppercase tracking-[0.28em] text-[#ffb066]">Offer Snapshot</div>
              <div className="mt-6 space-y-4">
                {[offer.duration, offer.expiresAt ? `Ends ${offer.expiresAt.slice(0, 10)}` : "", offer.description].filter(Boolean).map((text, index) => (
                  <div key={`${text}-${index}`} className="rounded-[24px] border border-white/10 bg-black/20 p-4">
                    <div className="text-sm leading-7 text-white/72">{text}</div>
                  </div>
                ))}
              </div>
            </div>
