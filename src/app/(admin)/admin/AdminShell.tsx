...
function isReadyPlanStudioPath(pathname: string) {
  return pathname === "/admin/ready-plans" || pathname.startsWith("/admin/ready-plans/");
}

export default function AdminShell({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  const isReadyPlanStudio = isReadyPlanStudioPath(pathname);
  ...
  return (
    <main className="min-h-screen bg-[#060606] text-white">
      ...
      <div className={`mx-auto py-4 md:py-8 ${isReadyPlanStudio ? "max-w-[1760px] px-2 md:px-4" : "max-w-[1440px] px-5 md:px-8"}`}>
        <div className="overflow-hidden rounded-[36px] border border-white/10 bg-black/35 shadow-[0_32px_120px_rgba(0,0,0,0.48)] backdrop-blur-2xl">
          <div className="border-b border-white/10 px-5 py-5 md:px-6">
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <Link
                  href="/profile"
                  className="rounded-2xl border border-[#ff7a00]/35 bg-[#ff7a00]/10 px-4 py-3 text-sm font-semibold text-[#ffbf82] transition hover:bg-[#ff7a00]/16 hover:text-white"
                >
                  Admin Profile
                </Link>
          ...
          <div className={isReadyPlanStudio ? "block" : "grid gap-0 lg:grid-cols-[280px_1fr]"}>
            {!isReadyPlanStudio ? (
            ...
            <section className={isReadyPlanStudio ? "p-2 md:p-4 xl:p-5" : "p-5 md:p-6"}>{children}</section>
          </div>
        </div>
      </div>
    </main>
