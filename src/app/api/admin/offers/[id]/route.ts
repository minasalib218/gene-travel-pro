...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  ...
  return NextResponse.json({ ok: true, offer: parseOfferLiveRecord(row as any) });
}
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    const payload = buildOfferLiveData({ ...(existing as any), ...(await req.json().catch(() => ({}))) });
    const updated = await prisma.offer.update({ where: { id: params.id }, data: payload });
    revalidateOfferPaths(updated.slug);
    return NextResponse.json({ ok: true, offer: parseOfferLiveRecord(updated as any) });
  } catch (error: any) {
...
export async function DELETE(_: Request, { params }: { params: { id: string } }) {
  ...
  const existing = await prisma.offer.findUnique({ where: { id: params.id } });
  if (!existing) return NextResponse.json({ ok: false, code: "NOT_FOUND" }, { status: 404 });
  await prisma.offer.update({ where: { id: params.id }, data: { status: "removed" } });
  revalidateOfferPaths(existing.slug);
