...
import LemonCheckoutButton from "@/components/payments/LemonCheckoutButton";
import { getPlanRules, type GenePlanType } from "@/lib/credits/planRules";
import { trackAnalyticsEvent, trackCheckoutStart, trackPurchaseSuccess } from "@/lib/analytics";
...
export default function PricingPage() {
  ...
  const [paymentFlowState, setPaymentFlowState] = useState<"idle" | "checking" | "active" | "failed" | "unknown">("idle");
  const [paymentFlowMessage, setPaymentFlowMessage] = useState<string | null>(null);
  ...
  const completedPaymentId = useMemo(() => {
    const paymentState = searchParams.get("payment");
    const paymentId = searchParams.get("paymentId");
    return paymentState === "success" && paymentId ? paymentId : null;
  }, [searchParams]);

  useEffect(() => {
    if (!completedPaymentId) {
      setPaymentFlowState("idle");
      setPaymentFlowMessage(null);
      return;
    }
    ...
    async function pollStatus() {
      ...
      setPaymentFlowMessage("Payment received. We’re activating your Gene pass now.");

      try {
        const res = await fetch(`/api/payment/status?paymentId=${encodeURIComponent(completedPaymentId)}`, {
          cache: "no-store",
        });
        const json = (await res.json().catch(() => null)) as
          | {
        ...
        if (json?.passStatus === "ACTIVE") {
          setPaymentFlowState("active");
          setPaymentFlowMessage("Your pass is live. Sending you to the AI Planner now.");

          if (!purchaseTracked) {
            purchaseTracked = true;
            trackPurchaseSuccess(json.planType || "Gene Travel", json.amount ?? 0, json.currency || "USD", {
              paymentId: completedPaymentId,
              orderId: json.providerOrderId || undefined,
              transactionId: completedPaymentId,
              provider: "lemonSqueezy",
              planType: json.planType || undefined,
              source: "pricing_popup_checkout",
            });
          }

          timeout = setTimeout(() => {
            router.replace("/start-planning");
          }, 1400);
          return;
        }
        ...
        if (json?.status === "FAILED" || json?.status === "REFUNDED") {
          ...
          setPaymentFlowMessage("Payment was not completed, so no credits were activated.");
          return;
        }

        if (json?.status === "UNKNOWN" || Date.now() - startedAt > 30000) {
          setPaymentFlowState("unknown");
          setPaymentFlowMessage("We couldn’t confirm activation yet. Please check your profile in a moment.");
          return;
        }
  ...
  return (
    <main className="relative min-h-screen overflow-hidden text-white">
      ...
      {paymentFlowState !== "idle" ? (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/55 px-5 backdrop-blur-md">
          <div className="w-full max-w-lg rounded-[30px] border border-white/12 bg-black/45 p-7 text-white shadow-[0_30px_110px_rgba(0,0,0,0.55)] backdrop-blur-2xl">
            <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full border border-white/10 bg-white/5">
              {paymentFlowState === "active" ? (
                <span className="text-2xl text-[#ff7a00]">✓</span>
              ) : paymentFlowState === "failed" || paymentFlowState === "unknown" ? (
                <span className="text-2xl text-[#ff7a00]">!</span>
              ) : (
                <span className="h-8 w-8 animate-spin rounded-full border-2 border-white/20 border-t-[#ff7a00]" />
              )}
            </div>
            ...
            <p className="mt-3 text-center text-sm leading-7 text-white/72">
              {paymentFlowMessage}
            </p>
            ...
            {(paymentFlowState === "failed" || paymentFlowState === "unknown") ? (
              <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                <button
                  ...
                  className="flex-1 rounded-2xl border border-white/12 bg-white/5 px-4 py-3 text-sm font-semibold text-white transition hover:bg-white/10"
                >
                  Stay on Pricing
                </button>
                <button
